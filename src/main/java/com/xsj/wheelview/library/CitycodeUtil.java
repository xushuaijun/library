...
//	}
	...
	public ArrayList<String> getProvince(List<CityData> provice) {
		...
		for (int i = 0; i < provice.size(); i++) {
			province_list.add(provice.get(i).getState_name());
			province_list_code.add(String.valueOf(provice.get(i).getState_code()));
		}
	...
	public ArrayList<String> getCity(
			HashMap<String, List<CityData>> cityHashMap, String provicecode) {
		...
		city.addAll(cityHashMap.get(provicecode));
		for (int i = 0; i < city.size(); i++) {
			city_list.add(city.get(i).getName());
			city_list_code.add(String.valueOf(city.get(i).getId()));
		}
		return city_list;
